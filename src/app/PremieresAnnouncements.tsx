...
export default function PremieresAndAnnouncements() {
  ...
  const {
    ...
    queryKey: ['premieres', 2025],
    queryFn: async () => {
      const data = await getPremieresByYear(2025);
      return [...data.filter((item: any) => item.release_date?.startsWith('2025')), ...data];
    },
    ...
    <section className="mt-16 px-8 w-full">
      ...
      {isLoading ? (
         <div className="relative">
          <div className="flex overflow-x-auto space-x-4 pb-4 px-12 scrollbar-hide">
        ...
        <p className="text-red-500">Failed to load premieres.</p>
      ) : (
        <div className="relative">
          <button
            onClick={() => scroll('left')}
